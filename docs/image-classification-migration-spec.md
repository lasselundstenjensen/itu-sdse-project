...
class SimpleCNN(nn.Module):
    def __init__(self, input_channels=3):
        ...
        # Convolutional layers for 128x128 images
        self.conv1 = nn.Conv2d(3, 32, kernel_size=3, stride=1, padding=1)
        self.conv2 = nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1)
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        
        # Calculate input size after convolutions and pooling
        # For 64x64 input: 64 -> 32 -> 16 (after 2 pool layers)
        # Feature map: 64 channels * 16 * 16
...
- **Input Size**: 64x64 RGB (resized from native 1600x768)
- **Loss**: `BCELoss` (Binary Cross Entropy Loss)
- **Optimizer**: `Adam` (default `lr=0.0001`)
